...
def handle_image_with_detection(frame, classes, masks, net, i):
    ...
    is_anomaly = postprocess(frame, classes, masks, outs)
...
def main():
    ...
    while True:
        ...
        frame, is_anomaly = handle_image_with_detection(image, classes, masks, net, i)
        if is_anomaly:
            ...
            c.execute(f'''
                INSERT INTO anomalies(date, file_path) VALUES(
                \"{f_date}\", \"{f_path}\")
            ''')
